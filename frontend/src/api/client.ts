...
export const api = {
  ...
  codingChallenges: () => request<{ challenges: CodingChallenge[] }>('/api/coding/challenges'),
  runCode: (moduleId: string | null, challengeId: string, code: string) => request<{ passed: boolean; tests: unknown[]; stderr?: string; error?: string; timeout?: boolean }>('/api/coding/run', { method: 'POST', body: JSON.stringify({ module_id: moduleId, challenge_id: challengeId, code }) }),
